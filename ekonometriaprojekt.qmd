---
title: "Vzťah medzi HDP a tvorbou investícií"
author: "Natália Neborásková"
format: html
editor: visual
---
...
#Vplyv HDP na hrubú tvorbu fixného kapitálu (Investície)
italy <- read_excel("zadanie.xlsx")
data<-ts(italy[, c("HDP", "investicie")], start=2005, frequency = 4)
colnames(data)[1]<-"hdp"
colnames(data)[2]<-"invest"
```
...
# Príprava dát pre ggplot (prevod z ts na dataframe)
data_df <- data.frame(
  time = as.numeric(time(data)),
  hdp = as.numeric(data[,"hdp"]),
  invest = as.numeric(data[,"invest"])
)

data_long <- data_df %>%
  pivot_longer(cols = c(hdp, invest), 
               names_to = "premenna", 
               values_to = "hodnota")
...
ggplot(data_long, aes(x = time, y = hodnota, color = premenna)) +
  ...
  labs(title = "Vývoj premenných — Taliansko 2005Q1–2025Q4",
       x = "Rok", 
       y = "Index",
...
```{r}
#ACF a PACF#ACdataF a PACF
acf2(data[,1],4) 
ggtsdisplay(data[,1])
#Ljung box test
LjungBoxTest <- function(res, k = 0, lag.max = 30, StartLag = 1, SquaredQ = FALSE){
...
LjungBoxTest(data[,1],lag.max = 4)
qchisq(0.95,4)   #9.487729
#alternativna funkcia pre Ljung-Box test
Box.test(data[,1],lag=4,type="Lj",fitdf = 0)
#zaver-> proces je NESTACIONARNY primame H1


#Dickeyho-fullerov test
summary(ur.df(data[,1],type="trend",lags=10,selectlags="BIC"))                      
summary(ur.df(data[,1],type="trend",lags=0))                                      
#nie je vhodny

#model s konstantou-drift-konstanta, pre model s driftom 
summary(ur.df(data[,1],type="drift",lags=10,selectlags="BIC"))
summary(ur.df(data[,1],type="drift",lags=0))                                      
# nie je vhodny

#model bez trendu a konstanty
summary(ur.df(data[,1],type="none",lags=10,selectlags="BIC"))
summary(ur.df(data[,1],type="none",lags=0))      
#prijmam H0 a tvrdime, že hdp je random walk - nestacionarny 


#stacionarita diferencii
summary(ur.df(diff(data[,1]),type="trend",lags=10,selectlags="BIC"))
summary(ur.df(diff(data[,1]),type="trend",lags=0))
...
ggtsdisplay(data[,2])
#Ljung box test
LjungBoxTest(data[,2],lag.max = 4)
qchisq(0.95,4)   #9.487729
#alternativna funkcia pre Ljung-Box test
Box.test(data[,2],lag=4,type="Lj",fitdf = 0)
#zaver-> proces je NESTACIONARNY primame H1


#Dickeyho-fullerov test
summary(ur.df(data[,2],type="trend",lags=10,selectlags="BIC"))                      
summary(ur.df(data[,2],type="trend",lags=0))                                        
#nie je vhodny


#model s konstantou-drift-konstanta, pre model s driftom nechame vybrat najvhodnejsi pocet lagov max 10-na zaklade schwarz kriteria(BIC)
summary(ur.df(data[,2],type="drift",lags=10,selectlags="BIC"))
summary(ur.df(data[,2],type="drift",lags=0))                                        
#model nie je vhodny 

#model bez trendu a konstanty
summary(ur.df(data[,2],type="none",lags=10,selectlags="BIC"))
summary(ur.df(data[,2],type="none",lags=0))      
#prijmam H0 a tvrdime, že rgnp je random walk - nestacionarny 


#stacionarita diferencii
summary(ur.df(diff(data[,2]),type="trend",lags=10,selectlags="BIC"))
summary(ur.df(diff(data[,2]),type="trend",lags=0))
#model je vhodny a prva diferencia je stacionarna - Model je integrovaný prveho radu I(1)


######samotne premenné sú nestacionarne ale oba maju prvú diferenciu integrovanu prveho radu
...
```{r}
#KOINTEGRACIA
#model dlhodobej rovnovahy-testovanie rezidualu, tabulky s kritickymi hodnotami

#Skumanie stacionarity rezidualu

#skumajme hdp~i - dlhodoba rovnica
(coint<-summary(lm(hdp~invest,data=data)))
summary(ur.df(resid(coint), type = "none", lags = 10, selectlags = "BIC")) 
summary(ur.df(resid(coint), type = "none", lags = 0)) 
#-3.5016 < -3,39 primame H1 -> medzi premennymi  je kointegracia na 5% hladine vyznamnosti
#model je kointegrovany
...
```{r}
#Tvorba ECM modelu
#vlozime rezidual do nasej databazy
my_ts_data <- ts(data, start=c(2005, 1), frequency=4)
res_ts <- ts(resid(coint), start=c(2005, 1), frequency=4)
data_final <- ts.union(my_ts_data, e = stats::lag(res_ts, -1))
colnames(data_final) <- c("hdp", "invest", "e")


# zakladny model ECM - vybereme parametre kt. su ŠV
rovECM <- dynlm(d(hdp) ~ d(invest) + L(e, 1), data = data_final)
summary(rovECM) #vse ŠV


#testovanie autokorelacie pomocou Ljung-Box
LjungBoxTest <- function(res, k = 0, lag.max = 30, StartLag = 1, SquaredQ = FALSE){
  stopifnot(k >= 0, StartLag >= 1, lag.max >= StartLag)
  n <- length(res)
  L0 <- StartLag
...
LjungBoxTest(resid(rovECM), lag.max = 10)
qchisq(0.95,10) 
#nemame autokorelaciu                                                           

```

```{r}
# model ECM resid
plot(resid(rovECM), type="l", col="purple") 
```
...
#prognoza
# 1. PRÍPRAVA ČISTÝCH VSTUPOV
# Použijeme na.omit, aby sme sa zbavili prípadných NA na konci dát
ciste_data <- na.omit(data_final)

posledne_e <- as.numeric(tail(ciste_data[,"e"], 1))
posledne_hdp <- as.numeric(tail(ciste_data[,"hdp"], 1))

# Výpočet priemerného prírastku investícií bez NA hodnôt
mean_d_invest <- mean(diff(ciste_data[,"invest"]), na.rm = TRUE)

# 2. DYNAMICKÝ VÝPOČET PROGNÓZY (4 kroky)
prog_d_hdp <- numeric(4)
aktualne_e <- posledne_e

for(i in 1:4) {
  # Používame koeficienty z tvojho modelu
  prog_d_hdp[i] <- -0.001157 + (0.426888 * mean_d_invest) + (-0.138652 * aktualne_e)
  
  # Aktualizácia rezídua pre ďalší krok (postupný návrat k rovnováhe)
  aktualne_e <- aktualne_e * (1 - 0.138652)
}

# 3. PREPOČET NA REÁLNE HODNOTY HDP
prognoza_hdp_hodnoty <- posledne_hdp + cumsum(prog_d_hdp)

# Vytvorenie časového radu pre výsledky
# Ak tvoje dáta končia v 4. kvartáli 2025, prognóza začne 1. kvartálom 2026
prognoza_ts <- ts(prognoza_hdp_hodnoty, 
                  start = c(2026, 1), 
                  frequency = 4)
...
# Graf pre vizuálnu kontrolu
plot(prognoza_ts, type="b", col="pink", lwd=2, 
     main="Prognóza vývoja HDP (2026)", 
     ylab="HDP", xlab="Obdobie")
